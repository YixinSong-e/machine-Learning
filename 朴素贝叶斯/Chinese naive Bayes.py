...
def spamTest():
    ...
    #print(vocabList)
    trainingSet = list(range(lengthOfFile))
    testSet = []  # 创建存储训练集的索引值的列表和测试集的索引值的列表
    for j in range(15320):  # 从50个邮件中，随机挑选出40个作为训练集,10个做测试集
        #randIndex = int(random.uniform(0, len(trainingSet)))  # 随机选取索索引值
        testSet.append(trainingSet[j])  # 添加测试集的索引值
        del (trainingSet[j])  # 在训练集列表中删除添加到测试集的索引值
        print(j)
    trainMat = []
